...
import { DynamicLeadingCandidateCard } from './DynamicLeadingCandidateCard';
...
function ResultUploadedCard({ percentage, uploadedResults, totalPollingUnits }: { percentage: number; uploadedResults: number; totalPollingUnits: number }) {
  return (
    <Card 
    ...
    >
      ...
      <CardContent>
        ...
        <p className="text-xs text-gray-500 mb-2">{uploadedResults.toLocaleString()} / {totalPollingUnits.toLocaleString()} Polling Units</p>
      </CardContent>
    </Card>
  );
}
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <Card 
            ...
            role="region"
            aria-label={`Last Update: ${formattedDate} ${formattedTime}`}
            aria-live="polite"
            aria-atomic="true"
          >
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
    ...
    <Card className="border-orange-200 bg-gradient-to-br from-orange-50 to-orange-100 hover:from-orange-100 hover:to-orange-150 transition-all duration-200 hover:shadow-md border rounded-xl shadow-sm">
      ...
      <CardContent>
        <div className="text-xl font-bold text-orange-900 mb-1">
          {registered > 0 ? ((totalVotes / registered) * 100).toFixed(2) : '0.00'}%
        </div>
...
export function DynamicStatsCards({ stateName, stats, polling, candidates }: DynamicStatsCardsProps) {
  ...
  // Calculate upload percentage dynamically from uploaded results and total polling units
  const calculatedUploadPercentage = polling?.totalPollingUnits && polling?.uploadedResults
    ? (polling.uploadedResults / polling.totalPollingUnits) * 100
    : 0;
  ...
  const topStats = [
    ...
    },
    {
      ...
      description: `Registered voters in ${stateName} State`,
  ...
  const electionSummaryStats = [
    ...
    {
      title: `Total Polling Units in ${stateName} State`,
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <ResultUploadedCard 
          percentage={calculatedUploadPercentage} 
          uploadedResults={polling?.uploadedResults || 0}
          totalPollingUnits={polling?.totalPollingUnits || 0}
        />
        <VoterTurnoutCard 
        ...
        />
        <LastUpdateCard />
        <DynamicLeadingCandidateCard stateName={stateName} candidates={candidates} />
      ...
      <div className="space-y-4">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-600 max-w-2xl mx-auto text-[16px] font-bold">
            Comprehensive metrics from {stateName} State election data collection and verification process
          </p>
        </div>
